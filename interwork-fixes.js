...
/* ─────────────────────────────────────────────────────────
   FIX: openEditProfileModal + saveProfile COM PAÍS
   ───────────────────────────────────────────────────────── */
window.openEditProfileModal = function() {
  const me = getUser(STATE.currentUserId);
  if (!me) return;

  // Gera as options de país a partir da sua lista de países
  const countryOptions = (window.COUNTRIES || []).map(c =>
    `<option value="${c.code}" ${me.country === c.code ? 'selected' : ''}>${c.flag} ${c.name}</option>`
  ).join('');
    ...
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-pop overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 overflow-y-auto space-y-5">
        ...
        <div class="flex items-center gap-4 p-4 bg-ink-50 rounded-2xl">
          <div class="relative">
            <img src="${me.avatar}" class="w-16 h-16 rounded-2xl object-cover border-2 border-white shadow-sm profile-avatar-img"/>
            <button type="button" onclick="changeProfileAvatar('${me.id}')"
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input id="ep-name" type="text" value="${escapeHtml(me.name || '')}"
              class="w-full bg-ink-50 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 transition"/>
          </div>

          <div>
            <label class="block text-xs font-bold text-ink-500 uppercase tracking-wider mb-1.5 ml-1">Bio / Headline</label>
            <textarea id="ep-bio" rows="3"
              class="w-full bg-ink-50 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 transition resize-none">${escapeHtml(me.bio || '')}</textarea>
          </div>

          <div>
            <label class="block text-xs font-bold text-ink-500 uppercase tracking-wider mb-1.5 ml-1">Country</label>
            <select id="ep-country"
              class="w-full bg-ink-50 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 transition appearance-none cursor-pointer">
      ...
      <div class="p-4 border-t border-ink-100 flex gap-3 shrink-0">
        <button onclick="closeModal()"
          class="flex-1 bg-ink-100 hover:bg-ink-200 text-ink-700 font-bold py-2 rounded-xl text-sm transition">
          Cancel
        </button>
        <button onclick="saveProfile()"
          class="flex-[2] bg-brand-500 hover:bg-brand-600 text-white font-bold py-2 rounded-xl text-sm flex items-center justify-center gap-2 transition">
          <i data-lucide="check" class="w-4 h-4"></i>Save changes
        </button>
      </div>
...
window.saveProfile = function() {
  ...
  me.name    = newName;
  me.bio     = bioEl?.value.trim()    || me.bio    || '';
  me.country = countryEl?.value       || me.country || '';
  me.skills  = skillsEl?.value
    ? skillsEl.value.split(',').map(s => s.trim()).filter(Boolean)
    : (me.skills || []);
  ...
  // Re-render o perfil se estiver nessa rota
  if (STATE.route === 'profile' && STATE.profileHandle === me.handle) {
    render();
  }
};
