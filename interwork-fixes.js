      ...
      <div class="flex-1 min-w-0">
        ...
        <div id="pane-gigs">
          ${services.length === 0
            ...
            : `<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">${services.map(s => serviceCard(s)).join('')}</div>`}
        </div>
...
/* ─────────────────────────────────────────────────────────
   FIX: VITRINE COM MOCKS + 4 COLUNAS + TRADUÇÕES (EN)
   ───────────────────────────────────────────────────────── */
window.viewExplore = function() {
  const query = (STATE.query || '').toLowerCase();
  const cat = STATE.category || 'all';
...
window.viewEarn = function() {
  return `<div class="max-w-4xl mx-auto px-5 py-16 text-center">
    <h1 class="text-4xl font-black text-ink-900 mb-6">Earn ITL</h1>
    <p class="text-ink-500 text-lg mb-12 max-w-2xl mx-auto">Complete tasks and get rewarded in ITL.</p>
    <div class="grid sm:grid-cols-2 gap-6 text-left">
      <div class="bg-white p-8 rounded-3xl border border-ink-100 shadow-sm">
        <h3 class="text-xl font-bold mb-3">Freelancing</h3>
        <p class="text-ink-500 text-sm mb-6">Post your services and get paid by clients worldwide.</p>
        <a href="#/new-service" class="text-brand-600 font-bold text-sm">Start selling →</a>
      </div>
      <div class="bg-white p-8 rounded-3xl border border-ink-100 shadow-sm">
        <h3 class="text-xl font-bold mb-3">Referrals</h3>
        <p class="text-ink-500 text-sm mb-6">Invite friends and earn rewards.</p>
        <span class="text-ink-300 font-bold text-sm">Coming soon</span>
      </div>
    </div>
  </div>`;
};
...
window.viewHire = function(serviceId) {
  ...
  return `<div class="max-w-3xl mx-auto p-6">
    ...
    <div class="bg-white border border-ink-100 rounded-3xl p-6 shadow-sm">
      <div class="mb-6">
        <label class="block text-xs font-bold text-ink-500 uppercase tracking-wider mb-2">Project Brief</label>
        <textarea id="hire-brief" class="w-full bg-ink-50 border-none rounded-2xl p-4 text-sm focus:ring-2 focus:ring-brand-500/20 transition h-32" placeholder="Describe exactly what you need..."></textarea>
      </div>
      <div class="flex items-center justify-between p-5 bg-brand-50 rounded-2xl mb-6">
        <div class="text-sm font-bold text-brand-700">Total Price</div>
      ...
      <button onclick="confirmHire('${s.id}')" class="w-full bg-brand-500 text-white font-black py-4 rounded-2xl shadow-pop transition">Confirm and Pay</button>
    </div>
  </div>`;
};
